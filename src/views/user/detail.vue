    ...
    export default {
        ...
        created() {
            ...
            _this.$http.get(`${_this.url}/admin`,data).then(resp=>{
                ...
                if(resp.code === 200) {
                    _this.user.account = resp.data[0].account
                    _this.user.nick_name = resp.data[0].nick_name
                    _this.user.created_at = resp.data[0].created_at
                    _this.user.login_password = resp.data[0].login_password
                    _this.user.status = String(resp.data[0].status)
                    _this.role.role_id = resp.data[0].role_id
                }else {
                    _this.$message({
